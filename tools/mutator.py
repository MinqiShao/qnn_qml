"""
pixel value transformation and affine transformation in DeepHunter
"""
...
class Seed():
    def __init__(self, ref_img, mutant, cl, pred, gt, m_times, l0_ref, linf_ref):
        self.cl = cl
...
class Mutator():
        ...
        for ii in range(try_num):
            if cl == 0:  # 0: can choose class A and B
                ...
                if tid in Mutator.classA:
                    ...
                    ref_img = torch.tensor(transformation(ref_img.clone().numpy(), param))
                    ref_img = ref_img.reshape(ori_shape)
                    return ref_img, img_new, 1, 1, l0_ref, linf_ref
            if cl == 1:  # 0: can choose pixel transform
                tid = random.sample(Mutator.classA, 1)[0]
                transformation = Mutator.transformations[tid]
                params = Mutator.params[tid]
                param = random.sample(params, 1)[0]
                img_new = torch.tensor(transformation(img.clone().numpy(), param))
                ...
                l0_new = l0_ref + torch.sum(sub != 0).item()
                linf_new = max(linf_ref, torch.max(torch.abs(sub)).item())
    ...
    #         ref_out, img_out, cl_out, changed, l0_ref, linf_ref = Mutator.mutate_one(ref_img, img, cl, seed.l0_ref, seed.linf_ref)
    #         if changed:
    ...
    #
    #     return torch.tensor(ref_batches), torch.tensor(batches), cl_batches, l0_ref_batches, linf_ref_batches