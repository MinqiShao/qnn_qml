...
def feature_reduc(train_x, test_x, f_type='resize', to_size=256):
    if f_type == 'resize':
        train_x = TF.resize(torch.unsqueeze(train_x, 1), (15, 15))
        test_x = TF.resize(torch.unsqueeze(test_x, 1), (15, 15))
...
def feature_redc_test(test_x, f_type='resize', to_size=256):
    if f_type == 'resize':
